#!/usr/bin/env bash
# =============================================================================
# config.sh — shared config & helpers for the Hopscotch deploy scripts.
# Not run directly; sourced by deploy-backend.sh / deploy-admin.sh /
# deploy-ecom.sh / deploy-all.sh.
# =============================================================================
...
ecom_image()    { echo "${ECR_REGISTRY}/${ECOM_REPO}:${IMAGE_TAG}"; }
...
NETWORK="${DOCKER_NETWORK:-hopscotch-net}"
ensure_network() {
...
status() {
  echo
  log "Current Hopscotch containers:"
  docker ps --filter "name=hopscotch-" \
    --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' || true
  echo
  docker stats --no-stream --format 'table {{.Name}}\t{{.CPUPerc}}\t{{.MemUsage}}' \
    $(docker ps --filter "name=hopscotch-" -q) 2>/dev/null || true
}
