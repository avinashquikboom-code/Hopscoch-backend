...
export class AdminService {
  ...
  async deleteWarehouse(warehouseId: any) {
    ...
    // Cascade delete dependent records (StockMovement & WarehouseInventory) then delete warehouse
    await prisma.$transaction([
      prisma.stockMovement.deleteMany({ where: { warehouseId: id } }),
      prisma.warehouseInventory.deleteMany({ where: { warehouseId: id } }),
      prisma.warehouse.delete({ where: { id } }),
    ]);

    logger.info(`Warehouse deleted successfully: ${id} (${existing.name})`);
